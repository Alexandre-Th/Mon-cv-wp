...
var app = {
  init: function() {
    ...
    if (window.matchMedia("(min-width: 1200px)").matches) {
      var rellax = new Rellax('.profil-content', {
        speed: 8
      });

      var rellax = new Rellax('.school-content', {
        speed: 4
      });
      var rellax = new Rellax('.spec-content', {
        speed: 6,
        center: true,
        round: true
      });
  ...
  handleEnterElement: function(element_id) {
    $('.main-nav a[href="#' + element_id + '"]').addClass('active');
  },
  handleLeaveElement: function(element_id) {
    $('.main-nav a[href="#' + element_id + '"]').removeClass('active');
  },
...
// fonction animation bouton burger + affichage menu
  $BtnBurger = $('.burger_menu-icon');
  $Menu = $('.menu');
  
  $($BtnBurger).ready(function(){
  
  $('.burger_menu-icon').click(function(e){
    e.preventDefault();

    if ($BtnBurger.hasClass('is-open')){
      $BtnBurger.addClass('is-close').removeClass('is-open');
      $Menu.addClass('close').removeClass('open');
    }
    else {
      $BtnBurger.removeClass('is-close').addClass('is-open');
